...
scheduler/    → APScheduler or Celery. Calls MCP tools via internal client or direct import.
...
```json
{
  ...
  "universe": ["SPY", "QQQ"],              // must be subset of STRATEGY_APPROVED_UNIVERSE
  "signals": [
...
- Any ticker not in `STRATEGY_APPROVED_UNIVERSE`
- `max_position_pct` > `RISK_MAX_POSITION_PCT`
- Unknown signal types
- Changed field count exceeds `STRATEGY_MAX_DIFF_FIELDS` vs current active version
- `confidence` < `STRATEGY_MIN_CONFIDENCE` (from proposal metadata)
...
- Minimum window: `STRATEGY_MIN_BACKTEST_DAYS` (default 252 trading days = ~1 year).
- Out-of-sample check: strategy must also pass on the 90-day period immediately before the in-sample window.
- Cost model (all configurable via env):
  - Fixed commission per trade
  - Slippage in bps
  - Spread proxy in bps
- Signal index must be shifted by 1 bar to prevent lookahead. Add a comment `# lookahead guard: shift(1)` at every such site.
- Document limitation: survivorship bias not corrected for ETF-only universe (acceptable for v1).
- Activation thresholds (configurable): Sharpe > 0.5, Max Drawdown < 25%, Win Rate > 40%.

---

## Market Hours

`core/timeutils.py` must export:
```python
is_market_open() -> bool
...
`paper_trade_tick` is a no-op outside market hours. Log a warning, do not raise.
...
Follow this sequence. Do not skip ahead. Run `pytest` after each phase.
...
6. **MCP server tools 1–3** — `ingest_latest_news`, `embed_and_upsert_docs`, `score_sentiment`, `query_kb`
